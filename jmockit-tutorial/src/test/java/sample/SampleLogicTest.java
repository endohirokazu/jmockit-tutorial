...
public class SampleLogicTest {
	...
	// @Mockedアノテーションをつけるとモック化してくれる。
	@Mocked
	SampleIF mockSampleIF;

	// @Mockedアノテーションをつけるとモック化してくれる。
	@Mocked
	SampleSingleton mockSampleSingleton;

	// randomメソッドだけモック化する
	@Mocked(methods="random")
	SamplePartial mockSamplePartial;
	
	@Before
	public void setUp() throws Exception {
		target = new SampleLogic();
		target.setSampleIF(mockSampleIF);
	}
	...
	public void testExecuteSamplePartial001() {
		...
		new Expectations() {
			{
				//mockSamplePartialの宣言のところで、random()だけモック化している。
				//constant()はモック化していないので、通常どおり処理される。
				mockSamplePartial.random();
				result = 10;
			}
		};
